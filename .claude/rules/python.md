...
- `main.py`: solo el menú. Funciones `leer`/`pedir_*` que envuelven `input()` (con `try/except ValueError` para números), funciones `mostrar_*` que imprimen tablas con f-strings y anchos fijos, una función `opcion_<nombre>(inventario)` de 5–15 líneas por opción, la lista `OPCIONES` de tuplas (tecla, texto, función) y `main()` con el `while True`. El `try/except ErrorInventario` está una sola vez, en el loop de `main()`: las `opcion_*` no lo repiten.
...
- `try/except` en los bordes: archivos en `persistencia.py`, red en `fuente_externa.py`, conversión de `input` en `main.py`, `ErrorInventario` en el loop de `main()`. Nunca `except:` a secas ni `except Exception`.
