...
3. `python -c "import modelos, inventario, persistencia, fuente_externa, analisis"` importa sin error. `grep -c "^def \|^    def " *.py` da 4 o más funciones/métodos con anotaciones.
4. Dirección de imports: ningún `import main` fuera de `main.py`; `modelos.py` no importa nada del proyecto; `pandas`/`matplotlib` solo en `analisis.py` y la notebook; `requests` solo en `fuente_externa.py` (`grep -n "^import\|^from" *.py`).
