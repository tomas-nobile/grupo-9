...
Los datos viven en dos archivos: `datos/productos.json` (el inventario) y `datos/movimientos.csv` (el historial). Se cargan al arrancar y se guardan después de cada cambio.
...
`[PENDIENTE]` Tabla: qué puede fallar · dónde se detecta · qué excepción · qué ve el usuario. Se completa a medida que aparecen casos (archivo inexistente, JSON roto, número inválido, código repetido, stock insuficiente, sin display para el gráfico).
...
`[PENDIENTE]` Se completa en F07.3. Mínimo: por qué POO si no se vio en clase; por qué JSON para productos y CSV para movimientos; por qué la regla de reposición es `stock_minimo * 2 - stock`; qué pasa si borro `datos/productos.json`; qué hace pandas que no haría con listas; qué cambiaría con más tiempo.
